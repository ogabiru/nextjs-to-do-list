...
export default async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    await db.all("SELECT * FROM todos WHERE id = ?", req.query.todoId, (err, rows) => {
      if (err) {
  ...
  if (req.method === 'PUT') {
    ...
    [req.body.title, req.body.description, req.body.started_at, req.body.paused_at, req.body.finished_at, req.body.time_spent, req.query.todoId],
    (err, rows) => {
      if (err) {
  ...
  if (req.method === 'DELETE') {
    await db.all("DELETE FROM todos WHERE id = ?", req.query.todoId, (err, rows) => {
      if (err) {
